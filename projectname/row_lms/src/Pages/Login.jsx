  ...
  const loginHandler = async(e)=>{
    ...
    try{
      ...
      if(response.status){
        ...
        Cookies.set('token', response.auth);
        nav('/dashboard');
      }
    }
    catch(error){
  ...
  return (
    ...
    <div className='bg-[#F5F7FF] w-full h-[100vh] flex justify-center items-center'>
        ...
        <form action="" onSubmit={loginHandler}>
            <input type="text" className=' mt-5 px-7 text-[16px] focus:outline-blue-400 w-full h-[50px] border border-1 border-[#c5c0c0]' placeholder='Username' onChange={(e)=>(setUserdata({...userdata, username : e.target.value}))}  />
            <input type="text" className=' mt-6 mb-5 px-7 text-[16px] focus:outline-blue-400 w-full h-[50px] border border-1 border-[#c5c0c0]' placeholder='Password' onChange={(e)=>(setUserdata({...userdata, password : e.target.value}))} />
